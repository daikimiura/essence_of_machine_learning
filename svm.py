...
class SVC:
    ...
    def fit(self, X, y, selections=None):
        ...
        while True:
            ...
            i = int(min(iydf[((y > 0) & (a > 0) | ((a < self.C) & (y < 0)))],
                        key=itemgetter(1))[0])
            j = int(max(iydf[((y < 0) | (a > 0) | ((a < self.C) & (y > 0)))],
                        key=itemgetter(1))[0])
            ...
            ai = ((1 - y[i] * y[j]
                   + y[i] * ((kij - kjj) * ay2 - (a[s] * y[s]*(kxi-kxj)).sum()))
                  / (kii + kjj - 2 * kij))
